...
execute "Keystone: add identity entpointTemplates" do
  ...
  node.set[:keystone][:adminURL] = "http://#{node[:controller][:ipaddress]}:5001/v2.0" 
  node.set[:keystone][:internalURL] = "http://#{node[:controller][:ipaddress]}:5000/v2.0"
...
execute "Keystone: add nova entpointTemplates" do
  # command syntax: endpointTemplates add 'region' 'service' 'publicURL' 'adminURL' 'internalURL' 'enabled' 'global'
  node.set[:nova][:adminURL] = "http://#{node[:controller][:ipaddress]}:8774/v1.1/%tenant_id%" 
...
execute "Keystone: add glance entpointTemplates" do
  # command syntax: endpointTemplates add 'region' 'service' 'publicURL' 'adminURL' 'internalURL' 'enabled' 'global'
  node.set[:glance][:adminURL] = "http://#{node[:controller][:ipaddress]}:#{node[:glance][:api_port]}/v1" 
